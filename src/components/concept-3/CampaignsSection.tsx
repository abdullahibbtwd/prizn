...
export function CampaignsSection({ lang }: CampaignsSectionProps) {
  const [featured, ...rest] = journalContent.campaigns
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          {featured && (
            ...
            >
              <Link
                to={getArticleBySourceId(featured.id)?.path ?? `/campaigns/${featured.id}`}
          ...
          <div className="flex flex-col gap-8 lg:col-span-5">
            {side.map((item, index) => (
              ...
              >
                <Link to={getArticleBySourceId(item.id)?.path ?? `/campaigns/${item.id}`} className="group block">
